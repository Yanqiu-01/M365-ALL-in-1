...
// Login failure budgets. A remote network peer keeps the strict policy. The
// loopback address is the on-box administration entry point: its threat model
// differs (the caller already controls the host) and a runaway local client
// replaying a stale password must not evict the real administrator for a
// quarter of an hour. Loopback therefore gets a looser threshold and a much
// shorter lockout - looser, never unlimited.
const (
	loginFailureThreshold         = 5
	loginLockoutWindow            = 15 * time.Minute
	loopbackLoginFailureThreshold = 20
	loopbackLoginLockoutWindow    = 2 * time.Minute
)

// loginPolicy is the failure budget applied to one login source.
type loginPolicy struct {
	Threshold int
	Window    time.Duration
}

// loginPolicyFor selects the budget for a request source.
func loginPolicyFor(loopback bool) loginPolicy {
	if loopback {
		return loginPolicy{Threshold: loopbackLoginFailureThreshold, Window: loopbackLoginLockoutWindow}
	}
	return loginPolicy{Threshold: loginFailureThreshold, Window: loginLockoutWindow}
}

// loginAllowed reports whether ip is currently outside a lockout window, and
// sweeps a stale entry when its window has decayed. It is deliberately no
// longer a gate in front of password verification: adminLogin verifies the
// password first so a correct credential always gets in. Callers use this only
// to observe lock state.
func (s *Server) loginAllowed(ip string, now time.Time) (bool, time.Duration) {
	...
	if a.WindowStart.IsZero() || now.Sub(a.WindowStart) > loginLockoutWindow {
...
// recordLoginFailure books one failed attempt against ip under policy p and
// reports whether the source is inside a lockout window, plus the remaining
// wait. The caller answers 429 when locked instead of 401, which keeps the
// lockout reply from disclosing that this particular attempt had a bad
// password.
//
// A failure arriving while the source is already locked is counted but never
// extends or restarts LockedUntil. Extending it was the defect: a client
// retrying every few seconds pushed the deadline forward forever, so the
...
func (s *Server) recordLoginFailure(ip string, now time.Time, p loginPolicy) (bool, time.Duration) {
	...
	if _, exists := s.loginAttempts[ip]; !exists && len(s.loginAttempts) >= maxLoginAttemptEntries {
		for key, attempt := range s.loginAttempts {
			if now.Sub(attempt.WindowStart) > loginLockoutWindow && now.After(attempt.LockedUntil) {
	...
	if a.WindowStart.IsZero() || now.Sub(a.WindowStart) > p.Window {
		a = loginAttempt{WindowStart: now}
	}
	a.Failures++
	if a.Failures >= p.Threshold {
		a.LockedUntil = now.Add(p.Window)
	}
	s.loginAttempts[ip] = a
	if now.Before(a.LockedUntil) {
		return true, a.LockedUntil.Sub(now)
