...
// 「购买代理」入口已按用户要求移除，因此不再断言它存在；免费拉取与批量删除
// 仍然必须在页面上。同时断言购买入口确实不再出现，避免被无意加回。
func TestProxyPoolDashboardHasImportAndSelectedDelete(t *testing.T) {
	...
	text := string(page)
	for _, marker := range []string{"proxyHunterImport()", "panelStartRegister()", "placeRegisterFrame()", "proxyDeleteSelected()", "proxySelectAll"} {
		if !strings.Contains(text, marker) {
			t.Fatalf("dashboard missing %q", marker)
		}
	}
	for _, removed := range []string{"proxyPurchaseURL", "proxyOpenPurchase"} {
		if strings.Contains(text, removed) {
			t.Fatalf("purchase entry point %q should have been removed", removed)
		}
	}
	// 只保留一个连通性检查入口：proxyTest() 那个按钮已删。
	if strings.Contains(text, "proxyTest()") {
		t.Fatal("duplicate connectivity test entry proxyTest() should have been removed")
	}
