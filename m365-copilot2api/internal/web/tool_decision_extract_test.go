...
func TestFinalDecisionFrameWinsOverEarlierCall(t *testing.T) {
	tools := extractTestTools()

	calls, parsed := parseModelToolDecision("CALL_TOOL: read_file({\"path\":\"a.py\"})\n```\nNO_TOOL_NEEDED\n```", tools, "auto")
	if !parsed || len(calls) != 0 {
		t.Fatalf("terminal no-tool must cancel earlier call: parsed=%v calls=%+v", parsed, calls)
	}

	calls, parsed = parseModelToolDecision("CALL_TOOL: read_file({\"path\":\"a.py\"})\nCALL_TOOL: unknown_tool({})", tools, "auto")
	if parsed || len(calls) != 0 {
		t.Fatalf("malformed final directive must not revive stale call: parsed=%v calls=%+v", parsed, calls)
	}

	calls, parsed = parseModelToolDecision("CALL_TOOL: read_file({\"path\":\"a.py\"})\n{\"calls\":null}", tools, "auto")
	if parsed || len(calls) != 0 {
		t.Fatalf("malformed final envelope must not revive stale call: parsed=%v calls=%+v", parsed, calls)
	}
}

func TestExplicitToolChoiceRejectsEmptyTerminalDecision(t *testing.T) {
	tools := extractTestTools()
	choices := []any{
		"required",
	...
	for _, choice := range choices {
		for _, text := range []string{"NO_TOOL_NEEDED", `{"calls":[]}`} {
			calls, parsed := parseModelToolDecision(text, tools, choice)
			if parsed || len(calls) != 0 {
				t.Fatalf("choice=%#v text=%q silently accepted empty decision: parsed=%v calls=%+v", choice, text, parsed, calls)
			}
		}
	}
}

func TestFinalEnvelopeMustValidateAsAWhole(t *testing.T) {
	tools := extractTestTools()
	text := `{"calls":[{"name":"read_file","arguments":{"path":"a.py"}},{"name":"unknown_tool","arguments":{}}]}`
	calls, parsed := parseModelToolDecision(text, tools, "auto")
	if parsed || len(calls) != 0 {
		t.Fatalf("partially invalid final envelope must enter repair path: parsed=%v calls=%+v", parsed, calls)
	}
}
