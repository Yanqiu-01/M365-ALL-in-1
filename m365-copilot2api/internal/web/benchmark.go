...
// benchmarkConcurrency 决定能力评测同时在跑的任务数。串行评测要跑完整套
// 任务耗时过长，而网关的账号池与出口池本就支持并发；取 4 是在缩短总时长
// 与避免把上游账号打进限流之间的折中。
const benchmarkConcurrency = 4
...
// benchRepeatHintThreshold 是「同参同结果的连续重复调用」开始提示的次数。
//
// 依据实测运行日志：shift 任务第 9-14 步连续 6 次 read_file 读同一个
// schedule.json，返回内容一字不差，每一次都是一整轮完整上游请求（8-28 秒）。
// 第 2 次重复还可能是模型在复核，第 3 次起就只是空转，因此阈值取 3。
const benchRepeatHintThreshold = 3

// benchRepeatHint 追加在重复工具结果之后，提示模型停止空转。
//
// 只做提示注入、不拒绝执行工具：直接拒绝会打断模型的推理链。历史上
// filterCompletedCalls 的绝对禁令（「任何已完成的调用都不得重复」）曾导致
// 模型在需要复跑 run_tests 时回 NO_TOOL_NEEDED，与网关的闭环提示自相矛盾，
// 反而把 14 步全部空转掉。
const benchRepeatHint = "\n\n[网关提示] 该调用与前一次完全相同且结果未发生变化。若任务已完成请直接给出最终回答，不要重复调用相同的工具。"

// benchRepeatTracker 记录最近一次工具调用的指纹（工具名 + 规范化参数 +
// 回传结果）。判定目标是「连续」重复，因此只需保留最近一次指纹：中间夹了
// 别的调用、或同一调用的结果发生了变化，都会重置计数。
type benchRepeatTracker struct {
	name   string
	args   string
	result string
	count  int
...
// observe 记录一次调用并返回它已连续重复的次数（首次出现返回 1）。
func (t *benchRepeatTracker) observe(name, args, result string) int {
	if t == nil {
		return 1
	}
	if t.name == name && t.args == args && t.result == result {
		t.count++
		return t.count
	}
	t.name, t.args, t.result, t.count = name, args, result, 1
	return 1
}

// benchChatCall 是评测工具循环取模型回复的唯一入口。抽成包级变量是为了让
// 步数收敛的回归测试能驱动一个脚本化的模型，而不必真打上游；生产路径直接
// 委派给 (*Server).benchChat，与 routerFailoverChat 的做法一致。
var benchChatCall = func(ctx context.Context, s *Server, model, effort string, messages []map[string]any) (map[string]any, time.Duration, error) {
	return s.benchChat(ctx, model, effort, messages)
}
...
func (s *Server) runBenchTask(ctx context.Context, task benchTask, model, effort string) benchTaskResult {
	...
	forcedClosures := 0
	// 同参同结果的连续重复调用计数，用于注入「停止空转」提示。
	repeated := &benchRepeatTracker{}
	...
	for step := 1; step <= benchMaxSteps; step++ {
		...
		response, elapsed, err := benchChatCall(ctx, s, model, effort, messages)
		if err != nil {
		...
		for _, raw := range calls {
			...
			if output, err := workspace.execute(name, parsed); err != nil {
				...
				payload = compactToolResult(err.Error(), 300)
			} else {
				...
				payload = compactToolResult(string(encoded), toolResultBudget(name, len(encoded)))
			}
			// 空转抑制：同名同参、且结果一字未变的连续重复只是在烧上游请求。
			// 指纹取「规范化后的参数」，使 {"path":"a.py"} 与 {"path":"a.py"}
			// 的不同书写形式判为同一调用（canonicalToolArguments）。
			if repeats := repeated.observe(name, canonicalToolArguments(arguments), payload); repeats >= benchRepeatHintThreshold {
				s.benchmark.logf("[%s]   %s 连续第 %d 次同参同结果重复，注入「停止空转」提示",
					task.ID, name, repeats)
				payload += benchRepeatHint
			}
			reply["content"] = payload
			messages = append(messages, reply)
		}

		// 收敛早退：闭环已达成就不再把剩余步数烧在重复调用上。
		if s.benchTaskConverged(task, workspace, step) {
			break
		}
	}
...
// benchTaskConverged 判定任务是否已经闭环达成，可以提前结束工具循环。
//
// 为什么需要它：此前工具循环只在模型返回「零工具调用」时才 break，因此只要
// 模型还愿意调工具，就会一路烧到 benchMaxSteps。实测 8 个任务全部精确跑满
// 14 步：debug 在第 5 步就 TESTS PASSED: 6/6，之后又调了 6 次 run_tests；
// shift 第 9-14 步连续 6 次读同一个 schedule.json。每一步都是一整轮上游请求，
// 既拖长总时长，也放大账号池负载。
//
// 判定必须落在 task.Grader 对「当前快照」的结论上，不能只看
// workspace.testsPass：后者只记录「最后一次 run_tests 时」的结论，而
// write_file 不会重置它。若模型跑通测试后又把代码改坏，只看 testsPass 就会
// 误判为已完成而提前收尾。因此两个条件必须同时成立。
//
// 编程类任务额外要求 run_tests 真的跑过并通过（与 benchCodingLoopPrompt 的
// 闭环要求一致）；推理类任务没有 run_tests 闭环，只以 Grader 满分为准。
//
// 早退只是「提前停止推进」：收尾处的 gradeBenchTask 防篡改校验与 Grader
// 打分完全不变，最终得分口径与跑满 14 步时一致。
func (s *Server) benchTaskConverged(task benchTask, workspace *benchWorkspace, step int) bool {
	if task.Grader == nil || workspace == nil {
		return false
	}
	snapshot := workspace.snapshot()
	// 受保护输入已被篡改时不早退：那不是「闭环达成」，把剩余步数留给模型
	// 改回来，否则收尾校验只会把任务判成 error。
	if gradeBenchTask(task.Protected, snapshot) != "" {
		return false
	}
	passed, total, _ := task.Grader(snapshot)
	if total <= 0 || passed != total {
		return false
	}
	if task.Category == "coding" {
		if testsPass, runs := workspace.testStatus(); !testsPass || runs == 0 {
			return false
		}
	}
	s.benchmark.logf("[%s] 第 %d 步闭环达成（%d/%d），提前结束，剩余 %d 步不再推进",
		task.ID, step, passed, total, benchMaxSteps-step)
	return true
}
...
func (s *Server) startBenchmark(model, effort string, ids []string) error {
	...
	safeGoWithCleanup("benchmark.run", func() {
		defer cancel()
		// 评测任务按 benchmarkConcurrency 并发执行。串行跑完整套任务耗时过长，
		// 而网关侧的账号池/出口池本就支持并发；结果按任务原顺序回填，
		// 保证前端的分类得分与任务列表次序与串行版本一致。
		results := make([]benchTaskResult, len(tasks))
		slots := make(chan struct{}, benchmarkConcurrency)
		...
		for index, task := range tasks {
			...
			wg.Add(1)
			slots <- struct{}{}
			go func(index int, task benchTask) {
				defer wg.Done()
				defer func() { <-slots }()
				defer func() {
					// 单条任务 panic 不应带走整轮评测：标记为错误并继续。
					if rec := recover(); rec != nil {
						results[index] = benchTaskResult{
							benchTask: task,
							Status:    "error",
							Error:     fmt.Sprintf("任务异常终止：%v", rec),
						}
					}
				}()
				...
				results[index] = s.runBenchTask(ctx, task, model, normalizedEffort)
			}(index, task)
		}
		wg.Wait()
