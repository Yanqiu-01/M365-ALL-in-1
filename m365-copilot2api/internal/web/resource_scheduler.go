...
// selectLeastInflight 是不依赖调度器状态的挑选：可用账号里在途最少的一个，同数
// 时按 ID 定序保证结果可复现。没有共享状态，因此无需加锁，也不会有竞争。
func selectLeastInflight(accounts []auth.AccountToken, available func(string) bool, inflight map[string]int) (auth.AccountToken, bool) {
	best := auth.AccountToken{}
	found := false
	for _, account := range accounts {
		...
		cur, prev := inflight[account.ID], inflight[best.ID]
		if cur < prev || (cur == prev && account.ID < best.ID) {
			best = account
		}
	}
	return best, found
...
func (s *resourceScheduler) Select(accounts []auth.AccountToken, available func(string) bool, inflight map[string]int) (auth.AccountToken, bool) {
	...
	// nil 调度器退化为无状态选择，而不是「无账号可用」。New() 一定会构造调度器，
	// 但测试和内部工具会直接拼 Server 字面量；早先这里让 nil 返回 false，于是
	// 那些调用方拿到的是「没有在线账号」这种误报，逼得调用点各自补一段惰性初始
	// 化 —— 那是在请求路径上无锁写共享字段。让 nil 自己可用，两个问题一起消失。
	if s == nil {
		return selectLeastInflight(accounts, available, inflight)
	}
