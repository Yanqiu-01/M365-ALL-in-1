...
// routerFrameSecretMarkers 是路由帧里需要打码的凭据前缀。
//
// 顺序即处理顺序：token 前缀在前、"Authorization:" 整头前缀在后，与恢复版
// 一致，避免 "Authorization: Bearer xxx" 先被折断后漏出真正的 token。
var routerFrameSecretMarkers = []string{"Bearer ", "bearer ", "access_token=", "accessToken=", "refresh_token=", "Authorization:"}

// truncateRouterFrame 归一化并打码一条诊断文本，最后截断到 4096 字节。
//
// 打码走 redactMarkedSecrets 的前向游标扫描：每个 marker 的每一次出现都会被
// 打码，且游标只前进，含 2 个以上 marker 的输入不再死循环。
func truncateRouterFrame(value string) string {
	...
	value = redactMarkedSecrets(value, routerFrameSecretMarkers, true)
	const limit = 4096
	if len(value) > limit {
		return value[:limit] + "…"
	}
