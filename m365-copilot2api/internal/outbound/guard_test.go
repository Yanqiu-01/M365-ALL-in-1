...
// The eviction backoff doubles and is capped at guardMaxBackoff (60 minutes).
...
func TestGuardTimingFromEnv(t *testing.T) {
	...
	// A fast cadence must stay reachable for debugging: the clamp floor is 5s.
	t.Setenv(EnvProxyGuardInterval, "10s")
	if got := guardTimingFromEnv(); got.base != 10*time.Second || got.suspect != 5*time.Second {
		t.Fatalf("debug timing = %s/%s, want 10s/5s", got.base, got.suspect)
	}
}

// The cadence invariants the lazy 15m default depends on: a suspect exit is
// re-probed faster than a live one, so eviction does not take evictAfter x base,
// and the evicted backoff keeps real headroom above the base cadence.
func TestGuardCadenceRelationships(t *testing.T) {
	timing := defaultGuardTiming()
	if timing.suspect >= timing.base {
		t.Fatalf("suspect %s must be shorter than base %s, otherwise eviction latency is evictAfter x base", timing.suspect, timing.base)
	}
	if timing.maxBackoff <= timing.base {
		t.Fatalf("maxBackoff %s must exceed base %s, otherwise the evicted backoff is meaningless", timing.maxBackoff, timing.base)
	}
	live := &poolEntry{state: stateLive}
	if got := live.probeIntervalLocked(timing); got != timing.base {
		t.Fatalf("live cadence = %s, want %s", got, timing.base)
	}
	// This is what keeps failover quick: the first failure demotes the exit to
	// suspect, and failures 2 and 3 then arrive on the suspect cadence.
	suspect := &poolEntry{state: stateSuspect}
	if got := suspect.probeIntervalLocked(timing); got != timing.suspect {
		t.Fatalf("suspect cadence = %s, want %s", got, timing.suspect)
	}
}
