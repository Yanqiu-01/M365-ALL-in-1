...
- 从最终 APK 提取出的 ARM64 `libm365.so` 可在 QEMU AArch64 环境启动 HTTP 服务；`/`、`/login`、管理员登录和鉴权后的 `/api/health` 均通过（可用 `build/android/qemu-smoke.sh` 重跑，不等同于真实 Android framework 测试）。
