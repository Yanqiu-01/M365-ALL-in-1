...
const flareWatchJS = `(function(){if(window.__m365Flare)return;window.__m365Flare=1;function val(fn){try{return String(fn()||'')}catch(e){return ''}}function set(id,v){var el=document.getElementById(id);if(!el||!v||el.value===v)return;el.value=v;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));}function layoutOnce(){if(window.__m365Laid)return;window.__m365Laid=1;var info=document.querySelector('.info-column');if(info)info.style.display='none';var shell=document.querySelector('.page-shell');if(shell){shell.style.display='block';shell.style.gridTemplateColumns='1fr';shell.style.width='100%';}var box=document.getElementById('turnstileBox')||document.querySelector('.form-card');if(box)box.scrollIntoView({block:'center'});}function fillOnce(){if(window.__m365Filled)return;if(!document.getElementById('username'))return;set('displayName',val(M365Flare.displayName));set('username',val(M365Flare.username));set('password',val(M365Flare.password));layoutOnce();window.__m365Filled=1;}function grab(){var el=document.querySelector('input[name=cf-turnstile-response]');var v=el&&el.value;if((!v||v.length<20)&&window.turnstile&&window.turnstile.getResponse){try{v=window.turnstile.getResponse()||v;}catch(e){}}if(v&&v.length>20){M365Flare.done(v);return true;}return false;}fillOnce();if(grab())return;setInterval(function(){fillOnce();grab();},1000);})();`
