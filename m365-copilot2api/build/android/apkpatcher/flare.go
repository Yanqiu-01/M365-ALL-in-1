...
`
	...
	files := map[string]string{
		...
		"FlareSolver$Place.smali":  flarePlaceSmali,
	}
...
const flareWatchJS = `(function(){if(window.__m365Flare)return;window.__m365Flare=1;function val(name){try{if(!window.M365Flare)return '';if(name==='displayName')return String(M365Flare.displayName()||'');if(name==='username')return String(M365Flare.username()||'');if(name==='password')return String(M365Flare.password()||'');}catch(e){}return ''}function set(id,v){var el=document.getElementById(id);if(!el||!v)return;el.focus();el.value=v;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));}function layout(){if(window.__m365Laid)return;window.__m365Laid=1;var info=document.querySelector('.info-column');if(info)info.style.display='none';var shell=document.querySelector('.page-shell');if(shell){shell.style.display='block';shell.style.gridTemplateColumns='1fr';shell.style.width='100%';}}function fill(){if(window.__m365Filled)return;if(!document.getElementById('username'))return;var d=val('displayName'),u=val('username'),p=val('password');if(!u||!p)return;var plan=document.getElementById('planId');if(plan&&plan.options&&plan.options.length&&!plan.value){plan.selectedIndex=0;plan.dispatchEvent(new Event('change',{bubbles:true}));}set('displayName',d);set('username',u);set('password',p);layout();window.__m365Filled=1;window.__m365FilledAt=Date.now();}function verified(){var el=document.querySelector('input[name=cf-turnstile-response]');var v=el&&el.value;return !!(v&&v.length>20)}function widget(){var box=document.getElementById('turnstileBox');if(!box||box.classList.contains('hidden'))return null;var t=box.querySelector('iframe');if(!t)return null;var src=t.getAttribute('src')||'';if(src&&src.indexOf('cloudflare')<0&&src.indexOf('turnstile')<0&&src.indexOf('challenges')<0)return null;var r=t.getBoundingClientRect();if(r.width<240||r.height<58)return null;return{t:t,r:r}}function clickBox(){if(verified()||!window.__m365Filled||window.__m365TapBusy)return;if(Date.now()-(window.__m365FilledAt||0)<1500)return;var w=widget();if(!w){window.__m365TsSeen=0;return}if(!window.__m365TsSeen)window.__m365TsSeen=Date.now();try{w.t.scrollIntoView({block:'center',inline:'nearest'})}catch(e){}if(Date.now()-window.__m365TsSeen<2500)return;if(window.__m365Clicked&&Date.now()-window.__m365Clicked<10000)return;window.__m365TapBusy=1;setTimeout(function(){window.__m365TapBusy=0;if(verified())return;var w2=widget();if(!w2)return;var r=w2.r;if(r.bottom<0||r.top>window.innerHeight)return;var x=r.left+22,y=r.top+32;try{M365Flare.tap(x,y)}catch(e){}window.__m365Clicked=Date.now()},600)}function submit(){if(window.__m365Submitted||!window.__m365Filled||!verified())return;var btn=document.getElementById('submitBtn');if(!btn||btn.disabled)return;var form=document.getElementById('registerForm');try{if(form&&form.requestSubmit)form.requestSubmit();else btn.click()}catch(e){try{btn.click()}catch(x){}}window.__m365Submitted=1;}function finish(){var msg=document.getElementById('message');var text=(msg&&(msg.textContent||'').trim())||'';var cls=(msg&&msg.className)||'';var btn=document.getElementById('submitBtn');if(/success/.test(cls)||/注册成功/.test(text)||(btn&&btn.classList.contains('login-ready'))){var m=text.match(/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+/);M365Flare.done('SUBMITTED:'+(m?m[0]:'registered-ok'));return true}if(/error/.test(cls)&&text.length>2&&!/正在创建/.test(text)){M365Flare.done('ERROR:'+text.slice(0,160));return true}return false}fill();setInterval(function(){fill();clickBox();submit();finish()},800)})();`
...
.method public place(FFFF)V
    ...
    new-instance v1, Lcom/m365/gateway/FlareSolver$Place;
    ...
    invoke-direct/range {v1 .. v6}, Lcom/m365/gateway/FlareSolver$Place;-><init>(Lcom/m365/gateway/FlareSolver;FFFF)V
...
const flarePlaceSmali = `.class Lcom/m365/gateway/FlareSolver$Place;
.super Ljava/lang/Object;
.source "FlareSolver.java"
...
.method constructor <init>(Lcom/m365/gateway/FlareSolver;FFFF)V
    .locals 0

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    iput-object p1, p0, Lcom/m365/gateway/FlareSolver$Place;->this$0:Lcom/m365/gateway/FlareSolver;

    iput p2, p0, Lcom/m365/gateway/FlareSolver$Place;->x:F

    iput p3, p0, Lcom/m365/gateway/FlareSolver$Place;->y:F

    iput p4, p0, Lcom/m365/gateway/FlareSolver$Place;->w:F

    iput p5, p0, Lcom/m365/gateway/FlareSolver$Place;->h:F
...
.method public run()V
    ...
    iget-object v0, p0, Lcom/m365/gateway/FlareSolver$Place;->this$0:Lcom/m365/gateway/FlareSolver;

    iget-object v1, v0, Lcom/m365/gateway/FlareSolver;->overlay:Landroid/widget/FrameLayout;

    if-eqz v1, :done

    iget-object v0, v0, Lcom/m365/gateway/FlareSolver;->activity:Landroid/app/Activity;
    ...
    iget v2, p0, Lcom/m365/gateway/FlareSolver$Place;->x:F

    mul-float/2addr v2, v0

    iget v3, p0, Lcom/m365/gateway/FlareSolver$Place;->y:F

    mul-float/2addr v3, v0

    iget v4, p0, Lcom/m365/gateway/FlareSolver$Place;->w:F

    mul-float/2addr v4, v0

    iget v5, p0, Lcom/m365/gateway/FlareSolver$Place;->h:F
