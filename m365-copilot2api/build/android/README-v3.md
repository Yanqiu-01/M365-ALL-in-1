...
- 从最终 APK 提取出的 `libm365.so` 在 QEMU AArch64 环境中启动成功：`/`、`/login`、管理员登录和鉴权后的 `/api/health` 均返回 200；可用 `build/android/qemu-smoke.sh` 重跑。
- 端点级原版对照：最终 APK 的 `/api/admin/models` 与原始 APK 的实测响应逐字段一致（14 项模型、全部元数据），`/api/admin/settings` 除监听端口外完全一致，`upstreamTones` 与 `codexModels` 一致，`/api/admin/models/sync` 与原版同为 404。这些断言已内置到 `build/android/qemu-smoke.sh` 和 `internal/web/apk_contract_test.go`（夹具取自原始二进制实测结果）。
