# M365 Copilot2API — 一份代码，两个构建目标

> ## 关于这个仓库
>
> 这是 [HEXUXIU/M365-Copilot2API](https://github.com/HEXUXIU/M365-Copilot2API) 的合并仓库：
> **PC 端与安卓端共用同一份 Go 源码**，靠 build tag 在编译期分流，不再维护两个平行拷贝。
>
> | 目标 | 分流方式 | 产物 |
> |---|---|---|
> | PC（Windows） | 默认构建，`net_mobile.go` 不参与 | `.build-out/m365-gateway-pc.exe` |
> | PC（Linux / 容器） | 默认构建 | `Dockerfile` 镜像，监听 4141 |
> | 安卓（arm64） | `//go:build android` 生效 | `libm365.so` → 重打包进 APK |
>
> `cmd/server/net_mobile.go` 带 `//go:build android`，只在安卓构建里编译；它的 `init()`
> 会把进程 DNS 换成硬编码公共解析器，这在 PC 上既不需要也不应该发生，所以 PC 构建必须
> 让该文件落在 tag 之外，而不是靠运行时判断。
>
> ### 构建命令
>
> 全部在 golang 容器内执行，不依赖宿主机工具链。本机可用镜像为 `golang:1.26` /
> `golang:1.26-alpine`（`go.mod` 仍声明 `go 1.23`，向下兼容，无需改动）。
>
> ```bash
> # 通用检查
> docker run --rm -v "$PWD:/src" -w /src golang:1.26 go build ./...
> docker run --rm -v "$PWD:/src" -w /src golang:1.26 go test ./internal/...
>
> # PC 端：交叉编译 Windows exe（含 vet + test，产物落在 .build-out/）
> docker run --rm -v "$PWD:/src" -w /src golang:1.26 sh build/pc/build-pc.sh
>
> # PC 端：Linux 容器镜像
> docker compose build
>
> # 安卓端：真实交叉编译 arm64 PIE（CGO_ENABLED=0，因此不需要 NDK）
> docker run --rm -v "$PWD:/src" -w /src -e CGO_ENABLED=0 -e GOOS=android \
>   -e GOARCH=arm64 -e GOARM64=v8.0 golang:1.26 \
>   go build -trimpath -buildvcs=false -buildmode=pie -o .build-out/libm365.so ./cmd/server
>
> # 安卓端：只验证 build tag 分流。CGO_ENABLED=0 是必须的 —— 否则 -tags android
> # 会在 linux 宿主上拉入 runtime/cgo，报 android/log.h: No such file or directory。
> docker run --rm -v "$PWD:/src" -w /src -e CGO_ENABLED=0 golang:1.26 \
>   go vet -tags android ./cmd/server
>
> # 安卓端：APK 重打包（需 apktool/aapt2/keytool，签名密钥库不在仓库内）
> KS_PASS=你的口令 bash build/android/build-v3-fixed.sh /path/to/base.apk /path/to/out
> ```
>
> 签名密钥库**不在仓库内**，默认路径 `$HOME/.m365-gateway/m365-gateway-v2.jks`，可用 `KS`
> 覆盖。首次运行会自动生成，**请立刻备份到仓库之外** —— 密钥一旦丢失，后续版本无法覆盖安装。
>
> ### 目录职责
>
> ```
> cmd/server/     共享入口；net_mobile.go 用 //go:build android 分流
> internal/       共享业务代码，PC 与安卓完全同一份
> web/            共享前端（安卓构建时同步进 APK 的 assets/web/）
...
- Windows / Linux 均可；构建统一在 golang 容器内进行，见顶部「构建命令」
...
```powershell
.\.build-out\m365-gateway-pc.exe
```

可通过环境变量 `M365_LISTEN` 覆盖监听地址；非回环绑定需要显式开启，见「安全说明」。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `M365_LISTEN` | `127.0.0.1:4141` | 监听地址（Docker 镜像内置为 `0.0.0.0:4141`，compose 只对宿主发布 `127.0.0.1:4141`） |
| `M365_ADMIN_PASSWORD` | `admin123` | 管理员密码（首次登录强制修改） |
| `M365_DATA_DIR` | `~/.config/m365-copilot2api` | 数据目录（token、密钥、用量等集中存储；Docker 内置为 `/data`） |
...
```
m365-copilot2api/
├── cmd/server/            # 共享入口；net_mobile.go 带 //go:build android
├── internal/              # 共享业务代码（PC 与安卓同一份）
...
├── assets/web/            # APK 内嵌前端副本，与 web/ 同内容
├── build/
│   ├── pc/                # PC 端构建
│   │   └── build-pc.sh    #   容器内 vet + test + 交叉编译 windows/amd64
│   └── android/           # 安卓端构建
│       ├── build-v3-fixed.sh   # APK 重打包主脚本（v3）
│       ├── repack-v2.sh        # 旧版重打包流程
