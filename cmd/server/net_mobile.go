...
// 仅 Android 构建包含此文件。
//
// 其 init() 无条件运行，而 systemResolverUsable() 判定的是 /etc/resolv.conf：
// 该文件在 Windows 上不存在，判定必然为假，于是整个进程的 net.DefaultResolver
// 会被替换成硬编码的公共 DNS（223.5.5.5 等）。桌面端这会绕过系统解析，
// VPN 下发的内网域名与 hosts 记录随之失效——而这些正是桌面环境的常态。
// Android 上此接管是必要的（应用进程读不到可用的 resolv.conf）。
